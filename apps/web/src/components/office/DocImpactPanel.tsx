      ...
      <AnimatePresence>
        {open && <>
          ...
          <motion.aside initial={{ x: 440 }} animate={{ x: 0 }} exit={{ x: 440 }} transition={{ type: 'spring', damping: 28, stiffness: 260 }} className="fixed right-0 top-0 z-50 flex h-full w-full max-w-lg flex-col border-l border-black/10 bg-white shadow-2xl dark:border-white/10 dark:bg-[#111]">
            <header className="flex items-start justify-between gap-3 border-b border-black/10 p-5 dark:border-white/10"><div><div className="flex items-center gap-2 text-sm font-bold"><GitBranch className="h-4 w-4 text-purple-500" /> Impacto AXOS</div><p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Encuentra otros documentos que referencian el mismo BOM, routing, NCR, CAPA, cliente, proveedor o entidad AXOS.</p></div><button onClick={() => setOpen(false)} className="rounded-full p-2 text-gray-500 dark:text-gray-400 hover:bg-black/5 hover:text-black dark:hover:bg-white/10 dark:hover:text-white"><X className="h-4 w-4" /></button></header>
