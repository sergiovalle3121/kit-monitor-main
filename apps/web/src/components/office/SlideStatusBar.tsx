...
import { Activity, AlertTriangle, CheckCircle2, Database, Layers, MessageSquare, MonitorPlay, ShieldAlert, Sparkles, StickyNote, ZoomIn } from 'lucide-react';
...
export function SlideStatusBar({
  ...
  return (
    <footer className="h-10 flex-shrink-0 rounded-2xl border border-black/10 dark:border-white/10 bg-white/85 dark:bg-[#111]/85 backdrop-blur px-3 flex items-center gap-3 text-xs text-gray-600 dark:text-gray-300">
      ...
      {health.offCanvasObjects > 0 && <span className="inline-flex items-center gap-1 rounded-lg px-2 py-1 bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-300" title="Objetos fuera del canvas de la diapositiva"><AlertTriangle className="w-3.5 h-3.5" /> Off {health.offCanvasObjects}</span>}
      ...
      {health.smartObjectsContractPending > 0 && <span className="hidden md:inline-flex items-center gap-1 rounded-lg px-2 py-1 bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-300" title="Smart Objects AXOS pendientes de refresh/contrato"><Database className="w-3.5 h-3.5" /> AXOS {health.smartObjectsContractPending}</span>}
      ...
      {!readOnly && <button onClick={onOpenAnimations} className="hidden sm:inline-flex items-center gap-1 rounded-lg px-2 py-1 hover:bg-black/5 dark:hover:bg-white/10" title="Abrir animaciones"><Sparkles className="w-3.5 h-3.5" /> Anim{health.animatedObjects ? ` ${health.animatedObjects}` : ''}</button>}
