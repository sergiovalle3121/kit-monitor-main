  ...
  return (
    <div className="w-72 flex-shrink-0 flex flex-col rounded-2xl border border-black/10 dark:border-white/10 bg-white dark:bg-[#141414] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {!items.length && <p className="text-xs text-gray-500 dark:text-gray-400 px-2 py-4 text-center">Diapositiva vacía.</p>}
        {ordered.map((it) => (
          <div key={it.idx}
            draggable
            ...
            <button onClick={(e) => { e.stopPropagation(); onToggleVisible(it.idx); }} title={it.visible ? 'Ocultar' : 'Mostrar'} className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 flex-shrink-0">
            ...
            <span className="text-[10px] text-gray-500 dark:text-gray-400 hidden group-hover:inline flex-shrink-0">{it.type}</span>
            <button onClick={(e) => { e.stopPropagation(); onToggleLock(it.idx); }} title={it.locked ? 'Desbloquear' : 'Bloquear'} className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 flex-shrink-0">
            ...
            <span className="flex items-center gap-0.5 flex-shrink-0 opacity-0 group-hover:opacity-100">
              <button onClick={(e) => { e.stopPropagation(); onFront(it.idx); }} title="Traer al frente" className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 leading-none"><ChevronsUp className="w-3.5 h-3.5" /></button>
              <button onClick={(e) => { e.stopPropagation(); onForward(it.idx); }} title="Traer adelante" className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 leading-none"><ChevronUp className="w-3.5 h-3.5" /></button>
              <button onClick={(e) => { e.stopPropagation(); onBackward(it.idx); }} title="Enviar atrás" className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 leading-none"><ChevronDown className="w-3.5 h-3.5" /></button>
              <button onClick={(e) => { e.stopPropagation(); onBack(it.idx); }} title="Enviar al fondo" className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 leading-none"><ChevronsDown className="w-3.5 h-3.5" /></button>
            </span>
          </div>
        ))}
      </div>
