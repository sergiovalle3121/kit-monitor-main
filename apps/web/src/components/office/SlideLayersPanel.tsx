...
/** Panel de selección (capas): orden Z, visibilidad y bloqueo por objeto. */
export function SlideLayersPanel({ items, activeIdx, onSelect, onToggleVisible, onToggleLock, onForward, onBackward, onClose }: {
  items: LayerItem[];
  activeIdx: number;
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        ...
        {ordered.map((it) => (
          <div key={it.idx} onClick={() => onSelect(it.idx)}
            className={`group flex items-center gap-1.5 rounded-lg px-2 py-1.5 cursor-pointer transition-colors ${it.idx === activeIdx ? 'bg-blue-500/10 ring-1 ring-blue-500/40' : 'hover:bg-black/[0.04] dark:hover:bg-white/[0.05]'}`}>
            <button onClick={(e) => { e.stopPropagation(); onToggleVisible(it.idx); }} title={it.visible ? 'Ocultar' : 'Mostrar'} className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
            ...
            <span className="text-[10px] text-gray-400 hidden group-hover:inline">{it.type}</span>
            <button onClick={(e) => { e.stopPropagation(); onToggleLock(it.idx); }} title={it.locked ? 'Desbloquear' : 'Bloquear'} className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
