...
import {
  SheetFormatDialog,
  type NumberFmtPayload,
  type StylePayload,
} from "./SheetFormatDialog";
...
import {
  consolidateByPosition,
  consolidateByCategory,
} from "./sheets/consolidate";
...
import {
  estimateWorkbookStats,
  shouldEmitWorkbook,
  workbookPerformanceLabel,
  type SignatureState,
} from "@/lib/office/workbookPerformance";
import {
  AXOS_SHEET_CONNECTORS,
  buildAxosConnectorRefresh,
...
} from "@/lib/office/axosConnectors";
import {
  addSheetCommentReply,
  anchorTypeForRange,
  commentsForSelection,
  createSheetCommentThread,
  deleteSheetComment,
  formatSheetCommentSummary,
  reopenSheetComment,
  resolveSheetComment,
  type SheetCommentAnchorType,
  type SheetCommentThread,
} from "@/lib/office/sheetComments";
import {
  auditWorkbookFormulas,
  formatFormulaAuditSummary,
} from "@/lib/office/formulaAudit";
import {
  analyzeWorkbookHealth,
  deriveSheetSelectionStats,
  deriveWorkbookHealth,
  formatWorkbookHealthReport,
} from "@/lib/office/workbookHealth";
import { scanXlsxCompatibility } from "@/lib/office/xlsxCompatibility";
import {
  formatPivotRefreshReport,
  refreshStoredPivots,
} from "@/lib/office/pivotGovernance";
...
function isCellProtected(sheet: any, r: number, c: number): boolean {
  ...
  return (
    Array.isArray(protection.ranges) &&
    protection.ranges.some(
      (x: any) => x.locked !== false && rangeHasCell(x.range, r, c),
    )
  );
}
...
export function SheetEditor({
  ...
  const initSheets = sheetsOf(value)?.length
    ? (sheetsOf(value) as any[])
    : [DEFAULT_SHEET];
  ...
  useEffect(() => {
    const refresh = () => {
      try {
        ...
        const cd = rng
          ? (sheet?.celldata ?? []).find(
              (x: any) => x.r === rng.r1 && x.c === rng.c1,
            )
          : null;
        const v = cd?.v;
        setFormulaText(
          v && typeof v === "object" && v.f
            ? String(v.f)
            : String(rawOf(cd as any) ?? ""),
        );
      } catch {
        /* noop: selection is not ready while Fortune-Sheet mounts */
      }
    };
    refresh();
  ...
  function sheetCommentFromApi(c: any): SheetCommentThread {
    return {
      id: String(c.parentId || c.id),
      persistedId: String(c.id),
      sheetIndex: typeof c.slideIndex === "number" ? c.slideIndex : 0,
      range: c.rangeRef || "A1",
      anchorType: ((c.anchorType === "object" ? "chart" : c.anchorType) ||
        "range") as SheetCommentAnchorType,
      ...
      resolvedAt: c.resolvedAt
        ? new Date(c.resolvedAt).toISOString()
        : undefined,
      replies: [],
    };
  }

  function syncComments(next: SheetCommentThread[]) {
  ...
  useEffect(() => {
    if (!docId) return;
    apiFetch(
      `${API_BASE}/office-documents/${docId}/slide-comments?includeResolved=true`,
    )
      .then(async (r) => {
        ...
        const roots = rows.filter(
          (c: any) =>
            !c.parentId &&
            ["sheet", "cell", "range", "table", "pivot", "chart"].includes(
              c.anchorType,
            ),
        );
        const replies = rows.filter((c: any) => c.parentId);
        const next = roots.map((root: any) => ({
          ...sheetCommentFromApi(root),
          replies: replies
            .filter((r: any) => r.parentId === root.id)
            .map((r: any) => ({
              id: String(r.id),
              text: String(r.text || ""),
              author: r.authorEmail || undefined,
      ...
      .catch(() => {
        /* fallback local workbook comments */
      });
  }, [docId, emit]);
  ...
  function rebuildTableRegistry() {
    ...
    for (const t of tablesRef.current) {
      const sheet = sheets[t.sheetIndex];
      if (!sheet) continue;
      const rng = parseRange(t.range);
      if (!rng) continue;
      const map = new Map<string, any>(
        (sheet.celldata ?? []).map((cd: any) => [`${cd.r}_${cd.c}`, cd]),
      );
      const headers: string[] = [];
      for (let c = rng.c1; c <= rng.c2; c++) {
        const cd = map.get(`${rng.r1}_${c}`);
        const raw =
          cd?.v && typeof cd.v === "object" ? (cd.v.v ?? cd.v.m) : cd?.v;
        headers.push(String(raw ?? ""));
      }
      defs.push({
        name: t.name,
        sheetName: sheet.name || `Hoja ${t.sheetIndex + 1}`,
        r1: rng.r1,
        c1: rng.c1,
        r2: rng.r2,
        c2: rng.c2,
    ...
    () => ({
      beforeUpdateCell: (r: number, c: number, value: any): boolean => {
        ...
        if (isCellProtected(sheet, r, c)) {
          toast.error(
            "Celda protegida. Quita la protección de hoja o rango para editar.",
          );
          return false;
        }
  ...
  const addName = useCallback(
    (nr: NamedRange) => {
      namesRef.current = [...namesRef.current, nr];
  ...
  const removeName = useCallback(
    (nm: string) => {
      namesRef.current = namesRef.current.filter((n) => n.name !== nm);
      setNames(namesRef.current);
      emit();
    },
    [emit],
  ...
  const updateChart = useCallback(
    (c: ChartConfig) => {
      chartsRef.current = chartsRef.current.map((x) => (x.id === c.id ? c : x));
      setCharts(chartsRef.current);
      emit();
    },
    [emit],
  ...
  function selectionStats() {
    const sheet = sheetsRef.current[activeIndex()] ?? sheetsRef.current[0];
    return deriveSheetSelectionStats(
      sheet,
      selectionRange(),
      commentsRef.current.filter((c) => c.sheetIndex === activeIndex()),
    );
  }
  function commitFormulaBar() {
    ...
    try {
      wb.setCellValue(rng.r1, rng.c1, normalizeCellInput(formulaText));
      setEditMode("Listo");
    } catch {
      toast.error("No se pudo escribir en la celda seleccionada.");
    }
  }
  ...
  function shiftRows(at: number, delta: 1 | -1) {
    ...
    sheet.celldata = (sheet.celldata ?? [])
      ...
      .map((cd: any) =>
        cd.r >= at + (delta < 0 ? 1 : 0) ? { ...cd, r: cd.r + delta } : cd,
      );
  ...
  function shiftCols(at: number, delta: 1 | -1) {
    ...
    sheet.celldata = (sheet.celldata ?? [])
      ...
      .map((cd: any) =>
        cd.c >= at + (delta < 0 ? 1 : 0) ? { ...cd, c: cd.c + delta } : cd,
      );
  ...
  function upsertConnectorProtection(
    sheet: any,
    ...
    sheet.axosProtection = {
      ...
      ranges: [
        ...ranges.filter(
          (r: any) =>
            r.connectorId !== instance.id &&
            !(r.connectorType === instance.type && r.range === instance.range),
        ),
        next,
      ],
    };
  ...
  function insertAxosConnector(type: AxosConnectorType) {
    ...
    if (chartSuggestions.length) {
      chartsRef.current = [
        ...chartsRef.current.filter(
          (chart) => !chartSuggestions.some((next) => next.id === chart.id),
        ),
        ...chartSuggestions,
      ];
      setCharts(chartsRef.current);
    ...
    remount(sheets);
    window.setTimeout(
      () => toast.success(`${instance.label} insertado en ${built.range}.`),
      30,
    );
  }
  ...
  function refreshAxosConnectors() {
    if (readOnly) return;
    if (!connectorsRef.current.length) {
      toast.info("No hay conectores AXOS insertados en este libro.");
      return;
    }
    const sheets = clone(sheetsRef.current);
    ...
    for (const connector of connectorsRef.current) {
      ...
      sheet.row = Math.max(
        sheet.row ?? 100,
        built.table.celldata.reduce((m, cd) => Math.max(m, cd.r + 4), 0),
      );
      sheet.column = Math.max(
        sheet.column ?? 30,
        built.table.celldata.reduce((m, cd) => Math.max(m, cd.c + 3), 0),
      );
    ...
    remount(sheets);
    window.setTimeout(
      () => toast.success(`${count} conectores AXOS actualizados.`),
      30,
    );
  }
  ...
  function selectedAnchorType(): SheetCommentAnchorType {
    const range = selectionRange();
    const table = tablesRef.current.find(
      (t) => t.sheetIndex === activeIndex() && t.range === range,
    );
    if (table) return "table";
    const pivot = pivotsRef.current.find(
      (p) =>
        p.config?.sheetIndex === activeIndex() && p.config?.range === range,
    );
    if (pivot) return "pivot";
    const chart = chartsRef.current.find(
      (c) => c.sheetIndex === activeIndex() && c.range === range,
    );
    if (chart) return "chart";
    return anchorTypeForRange(range);
  }
  async function addSheetComment() {
    const text = window.prompt("Comentario para la celda/rango seleccionado:");
    if (!text?.trim()) return;
    const assignedTo =
      window.prompt("Asignar a usuario/email (opcional):")?.trim() || null;
    const range = selectionRange();
    const anchorType = selectedAnchorType();
    const anchorLabel = `${sheetsRef.current[activeIndex()]?.name || `Hoja ${activeIndex() + 1}`} · ${range}`;
    const local = createSheetCommentThread({
    ...
    if (docId) {
      try {
        const r = await apiFetch(
          `${API_BASE}/office-documents/${docId}/slide-comments`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              anchorType,
              slideIndex: local.sheetIndex,
              rangeRef: range,
              anchorLabel,
              assignedTo,
              text,
    ...
    toast.success(`Comentario agregado en ${local.range}.`);
  }
  function showSheetComments() {
    const mine = commentsRef.current.filter(
      (c) => c.sheetIndex === activeIndex() && !c.resolved,
    );
    if (!mine.length) {
      toast.info("No hay comentarios abiertos en esta hoja.");
      return;
    }
    window.alert(
      mine
        .map((c, i) => `${i + 1}. ${formatSheetCommentSummary(c)}`)
        .join("\n"),
    );
  }
  async function replySelectionComment() {
    const [first] = selectedCommentThreads();
    if (!first) {
      toast.info(
        `No hay comentarios abiertos exactamente en ${selectionRange()}.`,
      );
      return;
    }
    ...
    if (!text?.trim()) return;
    syncComments(
      addSheetCommentReply(commentsRef.current, first.id, text, author),
    );
    if (docId && first.persistedId) {
      try {
        const r = await apiFetch(
          `${API_BASE}/office-documents/${docId}/slide-comments`,
          {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              parentId: first.persistedId,
              anchorType: first.anchorType,
              slideIndex: first.sheetIndex,
              rangeRef: first.range,
              anchorLabel: first.anchorLabel,
              text,
            }),
          },
        );
        if (r.ok)
          syncComments(
            addSheetCommentReply(
              deleteSheetComment(commentsRef.current, "__none__"),
              first.id,
              "",
              author,
              new Date(),
              String((await r.json()).id),
            ),
          );
      } catch {
        /* local fallback */
  ...
  function resolveSelectionComments() {
    ...
    for (const thread of threads) {
      next = resolveSheetComment(next, thread.id);
      if (docId && thread.persistedId)
        void apiFetch(
          `${API_BASE}/office-documents/${docId}/slide-comments/${thread.persistedId}`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
  ...
  function reopenSelectionComments() {
    ...
    if (!threads.length) {
      toast.info(
        `No hay comentarios resueltos exactamente en ${selectionRange()}.`,
      );
      return;
    }
    let next = commentsRef.current;
    for (const thread of threads) {
      next = reopenSheetComment(next, thread.id);
      if (docId && thread.persistedId)
        void apiFetch(
          `${API_BASE}/office-documents/${docId}/slide-comments/${thread.persistedId}`,
          {
  ...
  function deleteSelectionComments() {
    ...
    if (!threads.length) {
      toast.info(`No hay comentarios exactamente en ${selectionRange()}.`);
      return;
    }
    if (
      !window.confirm(
        `Eliminar ${threads.length} comentario(s) de ${selectionRange()}?`,
      )
    )
      return;
    let next = commentsRef.current;
    for (const thread of threads) {
      next = deleteSheetComment(next, thread.id);
      if (docId && thread.persistedId)
        void apiFetch(
          `${API_BASE}/office-documents/${docId}/slide-comments/${thread.persistedId}`,
          { method: "DELETE" },
  ...
  function protectSheet(lock: boolean) {
    ...
    window.setTimeout(
      () =>
        toast.success(
          lock ? "Hoja protegida." : "Protección de hoja desactivada.",
        ),
      30,
    );
  }
  function protectSelectionRange() {
    ...
    sheet.axosProtection = {
      ...(sheet.axosProtection ?? {}),
      ranges: [
        ...(sheet.axosProtection?.ranges ?? []),
        { range, locked: true, createdAt: new Date().toISOString() },
      ],
    };
    remount(sheets);
  ...
  function clearRangeProtection() {
    const sheets = clone(sheetsRef.current);
    const sheet = sheets[activeIndex()] ?? sheets[0];
    if (!sheet?.axosProtection?.ranges?.length) {
      toast.info("No hay rangos protegidos en esta hoja.");
      return;
    }
    const range = selectionRange();
    sheet.axosProtection.ranges = sheet.axosProtection.ranges.filter(
      (x: any) => x.range !== range,
    );
    remount(sheets);
    window.setTimeout(
      () => toast.info(`Protección quitada para ${range} si existía.`),
      30,
    );
  }
  ...
  function applyValidation({
    ...
    const rng = parseRange(range);
    if (!rng) {
      toast.error("Rango inválido. Ej: A1:A10");
      return;
    }
    const sheets = clone(sheetsRef.current);
    const sheet = sheets[sheetIndex] ?? sheets[0];
    ...
    let msg = "";
    if (action === "mark") {
      const n = markInvalidCells(sheet, range, cfg);
      msg = n
        ? `${n} celda(s) no válida(s) marcada(s) en rojo.`
        : "No hay celdas que incumplan la regla.";
    } else if (action === "clear") {
      const n = clearDataVerification(sheet, range);
      msg = `Validación quitada de ${n} celda(s).`;
  ...
  function applyFreezeAt() {
    const v = window.prompt(
      "Inmovilizar filas/columnas ANTES de esta celda (ej.: C4 → filas 1-3 y columnas A-B):",
      "B2",
    );
    if (!v) return;
    const rng = parseRange(v);
    if (!rng) {
      toast.error("Celda inválida.");
      return;
    }
    const sheets = clone(sheetsRef.current);
    const sheet = sheets.find((s: any) => s.status === 1) ?? sheets[0];
    ...
    sheet.frozen = {
      type: "rangeBoth",
      range: {
        row_focus: Math.max(0, rng.r1 - 1),
        column_focus: Math.max(0, rng.c1 - 1),
      },
    };
    remount(sheets);
  }
  ...
  function applyMerge(doMerge: boolean) {
    ...
    const range = selectionRange();
    if (doMerge) {
      if (!mergeCells(sheet, range)) {
        toast.info("Selecciona dos o más celdas para combinar.");
        return;
      }
      remount(sheets);
      window.setTimeout(() => toast.info(`Celdas combinadas (${range}).`), 30);
    } else {
      ...
      window.setTimeout(
        () =>
          toast.info(
            n
              ? `${n} combinación(es) separada(s).`
              : "No hay combinaciones en la selección.",
          ),
        30,
      );
    }
  ...
  function applyAutoFilter(enable: boolean) {
    ...
    if (enable) {
      const range = selectionRange();
      if (!setAutoFilter(sheet, range)) {
        toast.error("Rango inválido.");
        return;
      }
      remount(sheets);
      window.setTimeout(
        () => toast.info(`Autofiltro activado (${range}).`),
        30,
      );
    } else {
      ...
      remount(sheets);
      window.setTimeout(
        () => toast.info(had ? "Autofiltro quitado." : "No había autofiltro."),
        30,
      );
    }
  }
  ...
  const activeSheetLive = () =>
    liveData.find((s: any) => s?.status === 1) ?? liveData[0];
  function headerLabel(sheet: any, r: number, c: number): string {
    const cd = (sheet?.celldata ?? []).find((x: any) => x.r === r && x.c === c);
    const v = cd?.v;
    const disp =
      v == null ? "" : String((typeof v === "object" ? (v.m ?? v.v) : v) ?? "");
  ...
  function insertSlicer(kind: "slicer" | "timeline") {
    ...
    const rng = parseRange(selectionRange());
    if (!rng) {
      toast.error("Selecciona una columna con encabezado.");
      return;
    }
    const header = headerLabel(sheet, rng.r1, rng.c1);
    const range = `${colName(rng.c1)}${rng.r1 + 1}:${colName(rng.c1)}${rng.r2 + 1}`;
    if (kind === "slicer")
      sheet.slicers = [...(sheet.slicers ?? []), makeSlicer(range, 0, header)];
    else
      sheet.timelines = [
        ...(sheet.timelines ?? []),
        makeTimeline(range, 0, header),
      ];
    applySlicers(sheet);
    remount(sheets);
    setShowSlicers(true);
  ...
  const slicerValuesFor = (s: Slicer) =>
    slicerValues(activeSheetLive(), s.range, s.colRel);
  const toggleSlicerValue = (id: string, value: string) =>
    mutateSlicers((sheet) => {
      const s = (sheet.slicers ?? []).find((x: Slicer) => x.id === id);
      if (!s) return;
      const all = slicerValues(sheet, s.range, s.colRel);
      let sel = s.selected == null ? [...all] : [...s.selected];
      sel = sel.includes(value)
        ? sel.filter((x: string) => x !== value)
        : [...sel, value];
      s.selected = sel.length === all.length ? null : sel;
    });
  const clearSlicer = (id: string) =>
    mutateSlicers((sheet) => {
      const s = (sheet.slicers ?? []).find((x: Slicer) => x.id === id);
  ...
  const removeSlicer = (id: string) =>
    mutateSlicers((sheet) => {
      sheet.slicers = (sheet.slicers ?? []).filter((x: Slicer) => x.id !== id);
    });
  const setTimelineRange = (id: string, from: string, to: string) =>
    mutateSlicers((sheet) => {
      const t = (sheet.timelines ?? []).find((x: Timeline) => x.id === id);
      if (t) {
        t.from = from || null;
        t.to = to || null;
      }
  ...
  const removeTimeline = (id: string) =>
    mutateSlicers((sheet) => {
      sheet.timelines = (sheet.timelines ?? []).filter(
        (x: Timeline) => x.id !== id,
      );
    });
  ...
  async function copyRangeAsMarkdown() {
    const sheet =
      sheetsRef.current.find((s: any) => s.status === 1) ??
      sheetsRef.current[0];
    if (!sheet) return;
    const range = selectionRange();
    ...
    try {
      ...
      window.setTimeout(
        () => toast.info(`Tabla Markdown copiada (${range}).`),
        30,
      );
    } catch {
  ...
  function applyCondFormat(p: CondPayload) {
    ...
    if (!rng) {
      toast.error("Rango inválido. Ej: A1:B20");
      return;
    }
    const sheets = clone(sheetsRef.current);
  ...
  function applyData(mode: DataMode, payload: any) {
    ...
    if (mode === "sort") sortRangeMulti(sheet, payload);
      ...
      const n = removeDuplicates(sheet, payload);
      msg = `${Math.max(0, n)} fila(s) duplicada(s) eliminada(s).`;
    } else if (mode === "split") textToColumns(sheet, payload);
    else if (mode === "subtotal") {
      const n = applySubtotals(sheet, payload);
      msg = `${n} fila(s) de subtotal/total insertadas.`;
    } else if (mode === "spark") {
      if (!applySparkline(sheet, payload.dataRange, payload.cell, payload.type))
        msg = "Rango o celda inválidos.";
    } else if (mode === "fill") {
      const n = applyFill(sheet, payload);
      msg = `${n} celda(s) rellenada(s).`;
    } else if (mode === "transpose") {
      if (!transposeRange(sheet, payload.srcRange, payload.destCell))
        msg = "Rango o celda inválidos.";
    } else if (mode === "paste") {
      if (!copyRange(sheet, payload.srcRange, payload.destCell, payload.mode))
        msg = "Rango o celda inválidos.";
    } else if (mode === "filter") {
      ...
      if (!res) msg = "Rango inválido.";
        ...
        const n =
          sheets.filter((s: any) => /^Filtro/.test(s?.name ?? "")).length + 1;
        ...
        sheets.push({
          ...
          order: sheets.length,
          row: Math.max(100, res.matched + 12),
          column: Math.max(26, res.nCols + 4),
          config: {},
          status: 1,
        });
  ...
  function insertIntoCell(text: string): boolean {
    ...
    try {
      ...
      if (wb?.setCellValue) {
        wb.setCellValue(r, c, normalizeCellInput(text));
        return true;
      }
    } catch {
      /* fallback */
  ...
  function insertFunction(formula: string) {
    ...
    navigator.clipboard
      ...
      .then(() =>
        toast.success(
          `Función copiada: ${formula}  — pégala en la celda y completa los argumentos.`,
        ),
      )
      .catch(() => toast.info(`Escribe en la celda: ${formula}`));
  }
  function insertNameRef(ref: string) {
    if (!insertIntoCell(ref))
  ...
  const activeIndex = () => {
    const i = sheetsRef.current.findIndex((s: any) => s?.status === 1);
    return i >= 0 ? i : 0;
  };
  ...
  function refreshPivots() {
    ...
    if (!stored.length) {
      toast.info("No hay tablas dinámicas guardadas para actualizar.");
      return;
    }
  ...
  function doSpill() {
    ...
    if (!res.ok) {
      toast.error(res.error || "No se pudo derramar la matriz.");
      return;
    }
    remount(sheets);
    window.setTimeout(
      () =>
        toast.success(
          `Matriz derramada en ${res.rows}×${res.cols} celdas desde ${cell}.`,
        ),
      30,
    );
  }
  ...
  function doGoalSeek(p: GoalSeekPayload): { ok: boolean; text: string } {
    ...
    if (!Number.isFinite(tgt))
      return { ok: false, text: "El valor objetivo debe ser un número." };
    const sheets = clone(sheetsRef.current);
    const sheet = sheets[activeIndex()] ?? sheets[0];
    if (!sheet) return { ok: false, text: "No hay hoja activa." };
    const res = goalSeek(sheet, p.formulaCell, tgt, p.variableCell);
    if (!res.ok)
      return { ok: false, text: res.error || "No se encontró solución." };
    remount(sheets);
    return {
      ok: true,
      text: `${p.variableCell} = ${res.value} hace que ${p.formulaCell} ≈ ${res.result} (${res.iterations} iteraciones).`,
    };
  }
  ...
  const addScenario = useCallback(
    (sc: Scenario) => {
      scenariosRef.current = [
        ...scenariosRef.current.filter((s) => s.name !== sc.name),
        sc,
      ];
  ...
  const removeScenario = useCallback(
    (name: string) => {
      scenariosRef.current = scenariosRef.current.filter(
        (s) => s.name !== name,
      );
      setScenarios(scenariosRef.current);
      emit();
  ...
  function applyScenarioToActive(sc: Scenario) {
    ...
    remount(sheets);
    window.setTimeout(
      () => toast.success(`Escenario «${sc.name}» aplicado.`),
      30,
    );
  }
  function doScenarioSummary(resultCellsText: string): {
    ok: boolean;
    text: string;
  } {
    const cells = resultCellsText
      .split(/[,;\s]+/)
      .filter(Boolean)
      .map((s) => s.toUpperCase());
    if (!cells.length)
      return {
        ok: false,
        text: "Indica al menos una celda de resultado (p. ej. B5).",
      };
    if (!scenariosRef.current.length)
      return { ok: false, text: "No hay escenarios guardados." };
    const sheets = clone(sheetsRef.current);
    const sheet = sheets[activeIndex()] ?? sheets[0];
    ...
    const cd: any[] = [
      {
        r: 0,
        c: 0,
        v: { v: "Celda de resultado", m: "Celda de resultado", bl: 1 },
      },
    ];
    sum.headers.forEach((h, j) =>
      cd.push({ r: 0, c: j + 1, v: { v: h, m: h, bl: 1 } }),
    );
    sum.rows.forEach((row, i) => {
      cd.push({ r: i + 1, c: 0, v: { v: row.cell, m: row.cell, bl: 1 } });
      row.values.forEach((v, j) =>
        cd.push({ r: i + 1, c: j + 1, v: { v, m: String(v) } }),
      );
    });
    const n =
      sheets.filter((s: any) => /Resumen de escenarios/.test(s?.name ?? ""))
        .length + 1;
    pushResultSheet(
      sheets,
      `Resumen de escenarios ${n}`,
      cd,
      sum.rows.length + 3,
      sum.headers.length + 3,
    );
    remount(sheets);
    return {
      ok: true,
      text: `Resumen generado en una hoja nueva (${cells.length} celda(s) × ${sum.headers.length} escenario(s)).`,
    };
  }
  ...
  function readRangeGrid(sheets: any[], spec: string): any[][] | null {
    ...
    if (bang >= 0) {
      sheetName = spec.slice(0, bang).replace(/^'|'$/g, "");
      rangeStr = spec.slice(bang + 1);
    }
    const sheet = sheetName
      ? sheets.find((s: any) => (s.name || "") === sheetName)
      : (sheets[activeIndex()] ?? sheets[0]);
  ...
  function doConsolidate(p: ConsolidatePayload): { ok: boolean; text: string } {
    const sheets = clone(sheetsRef.current);
    const specs = p.ranges
      .split(/[\n;]+/)
      .map((s) => s.trim())
      .filter(Boolean);
    const tables: any[][][] = [];
    for (const spec of specs) {
      const g = readRangeGrid(sheets, spec);
      if (!g) return { ok: false, text: `Rango inválido: ${spec}` };
      tables.push(g);
    }
    const result =
      p.mode === "category"
        ? consolidateByCategory(tables, p.agg)
        : consolidateByPosition(tables, p.agg);
    if (!result.length)
      return { ok: false, text: "No hay datos para consolidar." };
    const cd: any[] = [];
    ...
    const n =
      sheets.filter((s: any) => /Consolidado/.test(s?.name ?? "")).length + 1;
    pushResultSheet(
      sheets,
      `Consolidado ${n}`,
      cd,
    ...
    return {
      ok: true,
      text: `Consolidado de ${tables.length} rango(s) en «Consolidado ${n}».`,
    };
  }
  ...
  function doSolve(p: SolverPayload): { ok: boolean; text: string } {
    ...
    const sheet = sheets[activeIndex()] ?? sheets[0];
    if (!sheet) return { ok: false, text: "No hay hoja activa." };
    const cells = p.variables
      .split(/[,;\s]+/)
      .filter(Boolean)
      .map((s) => s.toUpperCase());
    if (!cells.length)
      return { ok: false, text: "Indica al menos una celda variable." };
    ...
    (p.bounds || "")
      ...
      .filter(Boolean)
      .forEach((b) => {
        const m = /^([A-Za-z]+\d+)\s*(<=|>=|≤|≥)\s*(-?\d+(?:[.,]\d+)?)$/.exec(
          b,
        );
        if (!m) return;
        const cell = m[1].toUpperCase();
        const val = Number(m[3].replace(",", "."));
        const cur = bounds.get(cell) || {};
        if (m[2] === ">=" || m[2] === "≥") cur.min = val;
        else cur.max = val;
        bounds.set(cell, cur);
      });
    const variables: SolverVar[] = cells.map((c) => ({
      cell: c,
      ...(bounds.get(c) || {}),
    }));
    ...
    return {
      ok: true,
      text: `Óptimo: ${res.values.map((v) => `${v.cell}=${v.value}`).join(", ")} → objetivo ${res.objective} (${res.iterations} it).`,
    };
  }
  ...
  function doAutoSum(fn: AggFn) {
    ...
    if (!plan) {
      toast.error("Selecciona un rango para la autosuma.");
      return;
    }
    ...
    if (t && wb?.setCellValue) {
      try {
        wb.setCellValue(t.r1, t.c1, plan.formula);
        toast.success(`${plan.formula} → ${plan.targetCell}`);
        return;
      } catch {
    ...
    navigator.clipboard
      ?.writeText(plan.formula)
      .then(() => toast.info(`Copiado: ${plan.formula}`))
      .catch(() => toast.info(plan.formula));
  }
      ...
      for (let r = rng.r1; r <= rng.r2; r++)
        for (let c = rng.c1; c <= rng.c2; c++) {
          ...
          const raw =
            cd?.v && typeof cd.v === "object" ? (cd.v.v ?? cd.v.m) : cd?.v;
          const n = Number(raw);
          if (Number.isFinite(n) && raw !== "" && raw != null) out.push(n);
        }
      return out;
    }
    return text
  ...
  function pushResultSheet(
    ...
    sheets.push({
      ...
      celldata,
      order: sheets.length,
      row: Math.max(100, rows + 8),
      column: Math.max(26, cols + 4),
      config: {},
      status: 1,
    });
  }
  function doDataTable(p: DataTablePayload): { ok: boolean; text: string } {
    ...
    if (!colVals.length)
      return { ok: false, text: "No hay valores de entrada válidos." };
    const n =
      sheets.filter((s: any) => /Tabla de datos/.test(s?.name ?? "")).length +
      1;
    const name = `Tabla de datos ${n}`;
    if (p.mode === "one") {
      const res = dataTable1(sheet, p.formulaCell, p.colInputCell, colVals);
      if (!res.ok || !res.results)
        return { ok: false, text: res.error || "No se pudo calcular." };
      const cd: any[] = [
        { r: 0, c: 0, v: { v: "Valor", m: "Valor", bl: 1 } },
        { r: 0, c: 1, v: { v: "Resultado", m: "Resultado", bl: 1 } },
      ];
      colVals.forEach((v, i) => {
        cd.push(
          { r: i + 1, c: 0, v: { v, m: String(v) } },
          {
            r: i + 1,
            c: 1,
            v: { v: res.results![i], m: String(res.results![i]) },
          },
        );
      });
      ...
      if (!rowVals.length)
        return { ok: false, text: "No hay valores de fila válidos." };
      const res = dataTable2(
        sheet,
        p.formulaCell,
        p.rowInputCell,
        p.colInputCell,
      ...
      if (!res.ok || !res.matrix)
        return { ok: false, text: res.error || "No se pudo calcular." };
      const cd: any[] = [];
      colVals.forEach((cv, j) =>
        cd.push({ r: 0, c: j + 1, v: { v: cv, m: String(cv), bl: 1 } }),
      );
      rowVals.forEach((rv, i) => {
        ...
        colVals.forEach((_, j) =>
          cd.push({
            r: i + 1,
            c: j + 1,
            v: { v: res.matrix![i][j], m: String(res.matrix![i][j]) },
          }),
        );
      });
  ...
  function selectionRange(): string {
    try {
      ...
      if (first?.row && first?.column) {
        const r1 = first.row[0] ?? 0,
          r2 = first.row[1] ?? r1;
        const c1 = first.column[0] ?? 0,
          c2 = first.column[1] ?? c1;
  ...
  function auditFormulas() {
    ...
    if (!result.total) {
      toast.info("No hay fórmulas para auditar en este libro.");
      return;
    }
  ...
  function applyTable({ sheetIndex, opts }: TableStylePayload) {
    ...
    if (n && opts.hasHeader !== false && parseRange(opts.range)) {
      ...
      tablesRef.current = [
        ...tablesRef.current,
        { name: created, sheetIndex, range: opts.range },
      ];
      rebuildTableRegistry();
    }
    remount(sheets);
    if (!n)
      window.setTimeout(() => toast.error("Rango inválido para la tabla."), 30);
    else if (created)
      window.setTimeout(
        () =>
          toast.success(
            `Tabla creada: ${created} — usa ${created}[Columna] en fórmulas.`,
          ),
        30,
      );
  }
  ...
  function applyPivot(
    cfg: PivotConfig,
    target: { mode: "new" | "cell"; cell?: string },
  ) {
    const sheets = clone(sheetsRef.current);
    ...
    if (!res.matrix.length) {
      toast.error(res.warnings[0] || "No se pudo generar la tabla dinámica.");
      return;
    }
    if (target.mode === "cell") {
      const origin = parseRange(target.cell || "A1");
      if (!origin) {
        toast.error("Celda destino inválida.");
        return;
      }
      ...
      const n =
        sheets.filter((s: any) => /Tabla dinámica/.test(s?.name ?? "")).length +
        1;
      ...
      sheets.push({
        name,
        celldata: pivotToCelldata(res, 0, 0),
        order: sheets.length,
        row: Math.max(100, res.nRows + 8),
        column: Math.max(26, res.nCols + 4),
        config: {},
        status: 1,
      });
      ...
      pivotsRef.current = [
        ...pivotsRef.current,
        { id: `pv_${Date.now().toString(36)}`, config: cfg, sheetName: name },
      ];
    }
    setShowPivot(false);
    remount(sheets);
    if (res.warnings.length)
      window.setTimeout(() => toast.info(res.warnings.join("\n")), 30);
  }
    ...
    <div className="h-full w-full flex flex-col bg-white dark:bg-[#0e0e0e]">
      <OfficeRibbon storageKey="ribbon:sheet">
          ...
          <RibbonTab id="home" label="Inicio" icon={Home}>
            <RibbonGroup label="Portapapeles">
              <RibbonButton
                icon={Clipboard}
                label="Copiar"
                onClick={() => runGridCommand("copy")}
              />
              <RibbonButton
                icon={Scissors}
                label="Cortar"
                onClick={() => runGridCommand("cut")}
              />
              <RibbonButton
                icon={ClipboardPaste}
                label="Pegar"
                onClick={() => runGridCommand("paste")}
              />
            </RibbonGroup>
            <RibbonSeparator />
            <RibbonGroup label="Número">
              <RibbonButton
                icon={DollarSign}
                label="Moneda"
                onClick={() => applyQuickNumberFormat('"$"#,##0.00', "USD")}
              />
              <RibbonButton
                icon={Percent}
                label="Porcentaje"
                onClick={() => applyQuickNumberFormat("0.0%")}
              />
              <RibbonButton
                icon={Hash}
                label="General"
                onClick={() => applyQuickNumberFormat("General")}
              />
            </RibbonGroup>
            <RibbonSeparator />
            <RibbonGroup label="Formato rápido">
              <RibbonButton
                ...
                label="Relleno AXOS"
                onClick={() =>
                  applyQuickStyle({ bg: "#ecfdf5", fc: "#065f46" })
                }
              />
              <RibbonButton
                icon={Eraser}
            ...
            <RibbonGroup label="Celdas">
              <RibbonButton
                icon={PlusCircle}
                label="Insertar fila"
                onClick={() => shiftRows(selectionOrigin().r, 1)}
              />
              <RibbonButton
                icon={MinusCircle}
                label="Eliminar fila"
              ...
              <RibbonButton
                ...
                onClick={() => shiftCols(selectionOrigin().c, 1)}
              />
              <RibbonButton
                icon={Trash2}
          ...
          <RibbonTab id="data" label="Datos">
            <RibbonGroup label="Herramientas de datos">
              <RibbonButton
                ...
                onClick={() => setTool("validation")}
              />
              <RibbonButton
                icon={Palette}
                label="Formato condicional"
                onClick={() => setTool("condformat")}
              />
              <RibbonButton
                icon={Hash}
                label="Copiar como Markdown"
                hideLabel={false}
                onClick={copyRangeAsMarkdown}
              />
            </RibbonGroup>
            <RibbonSeparator />
            <RibbonGroup label="Ordenar y filtrar">
              <RibbonButton
                icon={ArrowDownUp}
                label="Ordenar rango (multinivel)"
                onClick={() => setDataMode("sort")}
              />
              <RibbonMenuButton
                icon={Filter}
                label="Autofiltro"
                ...
                items={[
                  {
                    label: "Activar autofiltro (selección)",
                    onClick: () => applyAutoFilter(true),
                  },
                  {
                    label: "Quitar autofiltro",
                    onClick: () => applyAutoFilter(false),
                  },
                ]}
              />
              <RibbonButton
                icon={Filter}
                label="Filtrar a hoja"
                onClick={() => setDataMode("filter")}
              />
              <RibbonButton
                icon={Crosshair}
                label="Segmentación de datos"
                onClick={() => insertSlicer("slicer")}
              />
              <RibbonButton
                icon={CalendarRange}
                label="Escala de tiempo"
                onClick={() => insertSlicer("timeline")}
              />
              <RibbonButton
                icon={CopyMinus}
                label="Quitar duplicados"
                onClick={() => setDataMode("dedup")}
              />
              <RibbonButton
                icon={Columns3}
                label="Texto en columnas"
                onClick={() => setDataMode("split")}
              />
              <RibbonButton
                icon={Combine}
                label="Consolidar"
            ...
            <RibbonGroup label="Análisis de hipótesis">
              ...
              <RibbonButton
                icon={Layers}
                label="Administrador de escenarios"
                hideLabel={false}
            ...
            <RibbonGroup label="Rellenar y transponer">
              <RibbonButton
                ...
                label="Rellenar serie"
                onClick={() => setDataMode("fill")}
              />
              <RibbonButton
                icon={FlipVertical2}
                label="Transponer"
                onClick={() => setDataMode("transpose")}
              />
              <RibbonButton
                icon={ClipboardPaste}
            ...
            <RibbonGroup label="Vista">
              <RibbonMenuButton
                ...
                items={[
                  {
                    label: "Inmovilizar fila 1",
                    onClick: () => applyFreeze("row"),
                  },
                  {
                    label: "Inmovilizar columna A",
                    onClick: () => applyFreeze("column"),
                  },
                  {
                    label: "Inmovilizar fila 1 + columna A",
                    onClick: () => applyFreeze("both"),
                  },
                  {
                    label: "Inmovilizar hasta una celda…",
                    onClick: applyFreezeAt,
                  },
                  {
                    label: "Quitar inmovilización",
                    onClick: () => applyFreeze(""),
                  },
                ]}
              />
        ...
        {!readOnly && (
          <RibbonTab id="insert" label="Insertar">
            <RibbonGroup label="Tablas">
              ...
              <RibbonButton
                icon={RefreshCw}
                label="Actualizar tablas dinámicas"
                onClick={refreshPivots}
              />
            </RibbonGroup>
            <RibbonSeparator />
        ...
        {!readOnly && (
          <RibbonTab id="format" label="Formato">
            <RibbonGroup label="Celdas">
              <RibbonButton
                icon={Hash}
                label="Formato de número y estilos"
                hideLabel={false}
        ...
        <RibbonTab id="axos" label="AXOS" icon={Activity}>
          <RibbonGroup label="Conectores ERP/MES">
            <RibbonMenuButton
              icon={RefreshCw}
              label="Insertar conector"
              menuWidth={320}
              items={AXOS_SHEET_CONNECTORS.map((connector) => ({
            ...
            <RibbonButton
              icon={Table2}
              label="Inventario"
              onClick={() => insertAxosConnector("inventory_snapshot")}
            />
            <RibbonButton
              icon={Activity}
              label="OEE"
              onClick={() => insertAxosConnector("oee_by_line")}
            />
            <RibbonButton
              icon={RefreshCw}
              label="Refrescar"
              onClick={refreshAxosConnectors}
            />
          </RibbonGroup>
        </RibbonTab>
        ...
        {!readOnly && (
          <RibbonTab id="formulas" label="Fórmulas">
            <RibbonGroup label="Autosuma">
              <RibbonMenuButton
                ...
                items={[
                  ...
                  {
                    label: "Contar números",
                    onClick: () => doAutoSum("COUNT"),
                  },
            ...
            <RibbonGroup label="Biblioteca de funciones">
              ...
              <RibbonButton
                icon={Search}
                label="Auditar fórmulas"
                hideLabel={false}
                onClick={auditFormulas}
              />
            </RibbonGroup>
            <RibbonSeparator />
            <RibbonGroup label="Nombres definidos">
              <RibbonButton
                icon={Tag}
                label="Administrador de nombres"
                hideLabel={false}
        ...
        {!readOnly && (
          <RibbonTab id="review" label="Revisar">
            <RibbonGroup label="Edición">
              ...
              <RibbonButton
                icon={Activity}
                label="Salud del workbook"
                hideLabel={false}
                onClick={showWorkbookHealth}
              />
            </RibbonGroup>
            <RibbonSeparator />
            <RibbonGroup label="Comentarios">
              <RibbonButton
                icon={StickyNote}
                label="Nota de celda"
                onClick={() => setDataMode("note")}
              />
              <RibbonButton
                icon={MessageSquare}
                label="Comentar rango"
                onClick={addSheetComment}
              />
              <RibbonMenuButton
                icon={MessageSquare}
                label="Comentarios"
                ...
                items={[
                  {
                    label: "Ver comentarios abiertos",
                    onClick: showSheetComments,
                  },
                  {
                    label: "Responder comentario de la selección",
                    onClick: replySelectionComment,
                  },
                  {
                    label: "Resolver comentarios de la selección",
                    onClick: resolveSelectionComments,
                  },
                  {
                    label: "Reabrir comentarios de la selección",
                    onClick: reopenSelectionComments,
                  },
                  {
                    label: "Eliminar comentarios de la selección",
                    onClick: deleteSelectionComments,
                  },
                ]}
              />
            </RibbonGroup>
            ...
            <RibbonGroup label="Protección">
              <RibbonMenuButton
                ...
                items={[
                  {
                    label: "Proteger hoja activa",
                    onClick: () => protectSheet(true),
                  },
                  {
                    label: "Quitar protección de hoja",
                    onClick: () => protectSheet(false),
                  },
                  {
                    label: "Proteger rango seleccionado",
                    onClick: protectSelectionRange,
                  },
                  {
                    label: "Quitar protección del rango",
                    onClick: clearRangeProtection,
                  },
                ]}
              />
            </RibbonGroup>
      ...
      <div className="h-10 flex items-center gap-2 border-y border-black/5 dark:border-white/10 bg-gray-50/80 dark:bg-white/[0.03] px-3">
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" && !readOnly) commitFormulaBar();
            if (e.key === "Escape") setEditMode("Listo");
          }}
      ...
      {contextMenu && !readOnly && (
        <div
          className="fixed z-[70] w-56 rounded-2xl border border-black/10 dark:border-white/10 bg-white dark:bg-[#161616] p-1.5 shadow-2xl"
          style={{ left: contextMenu.x, top: contextMenu.y }}
        >
          {[
            ["Copiar", () => runGridCommand("copy")],
            ["Cortar", () => runGridCommand("cut")],
            ["Pegar", () => runGridCommand("paste")],
            [
              "Formato moneda",
              () => applyQuickNumberFormat('"$"#,##0.00', "USD"),
            ],
            ["Formato porcentaje", () => applyQuickNumberFormat("0.0%")],
            ["Limpiar formato", clearSelectedFormatting],
            ["Insertar fila", () => shiftRows(selectionOrigin().r, 1)],
            ["Eliminar fila", () => shiftRows(selectionOrigin().r, -1)],
            ["Insertar columna", () => shiftCols(selectionOrigin().c, 1)],
            ["Eliminar columna", () => shiftCols(selectionOrigin().c, -1)],
          ].map(([label, fn]) => (
            <button
              key={String(label)}
              onClick={() => {
              ...
              className="w-full rounded-xl px-3 py-2 text-left text-xs font-medium hover:bg-black/5 dark:hover:bg-white/10"
            >
              {String(label)}
            </button>
          ))}
          ...
          {(() => {
            ...
            return (
              <span className="truncate">
                Conteo {s.count} · Núm {s.nums} · Suma {s.sum.toLocaleString()}{" "}
                · Prom {s.average.toLocaleString()} · Min {s.min ?? "—"} · Max{" "}
                {s.max ?? "—"}
              </span>
            );
          })()}
          <span className="hidden lg:inline-flex rounded-full border border-black/10 dark:border-white/10 px-2 py-0.5 uppercase tracking-wide">
            {(() => {
              const p = currentWorkbookStats();
              return `${p.label} · ${p.cells.toLocaleString()} celdas · ${(p.approxJsonBytes / 1024).toFixed(0)} KB`;
            })()}
          </span>
        </div>
      ...
      <AnimatePresence>
        ...
        {showGoalSeek && (
          <SheetGoalSeek
            defaultFormulaCell={selectionRange().split(":")[0]}
            onApply={doGoalSeek}
        ...
        {showScenarios && (
          <SheetScenarios
            ...
            onApply={applyScenarioToActive}
            onSummary={doScenarioSummary}
            onClose={() => setShowScenarios(false)}
          />
        )}
        {showSolver && (
          <SheetSolver
            defaultObjective={selectionRange().split(":")[0]}
            onApply={doSolve}
...
function SheetWorkbenchInspector({
  ...
  const protectedRanges = sheets.flatMap((s: any, i: number) => [
    ...(s?.axosProtection?.sheetLocked
      ? [
          {
            sheet: s.name || `Hoja ${i + 1}`,
            range: "Hoja completa",
            locked: true,
          },
        ]
      : []),
    ...(Array.isArray(s?.axosProtection?.ranges)
      ? s.axosProtection.ranges.map((r: any) => ({
          ...r,
          sheet: s.name || `Hoja ${i + 1}`,
        }))
      : []),
  ...
  const tool =
    "w-full rounded-xl border border-black/10 dark:border-white/10 px-3 py-2 text-left hover:bg-black/5 dark:hover:bg-white/10 disabled:opacity-40";
  return (
    <aside className="hidden xl:flex w-[340px] shrink-0 flex-col border-l border-black/10 dark:border-white/10 bg-gray-50/80 dark:bg-[#101010]">
      <div className="border-b border-black/10 dark:border-white/10 p-3">
        <div className="text-[11px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
          AXOS Sheets Workbench v2
        </div>
        <div className="mt-1 flex items-center justify-between gap-2">
          <div className="text-sm font-semibold text-foreground">
            Inspector industrial
          </div>
          <span className="rounded-full border border-black/10 dark:border-white/10 px-2 py-0.5 text-[10px] font-semibold">
            {health.score}/100
          </span>
        </div>
      </div>
      <div className="flex gap-1 overflow-x-auto border-b border-black/10 dark:border-white/10 p-2">
        {tabs.map(([id, label]) => (
          <button
            key={id}
            onClick={() => onTab(id)}
            className={`rounded-lg px-2 py-1 text-[11px] font-semibold ${tab === id ? "bg-gray-900 text-white dark:bg-white dark:text-gray-900" : "text-gray-500 hover:bg-black/5 dark:hover:bg-white/10"}`}
          >
            {label}
          </button>
        ))}
      ...
      <div className="min-h-0 flex-1 overflow-auto p-3 text-xs text-gray-600 dark:text-gray-300">
        {tab === "workbook" && (
          <Panel title="Workbook Health">
            <Metric label="Hojas" value={health.sheets} />
            <Metric label="Celdas usadas" value={health.usedCells} />
            <Metric label="Fórmulas" value={health.formulas} />
            <Metric label="Charts" value={health.charts} />
            <Metric label="Pivots" value={health.pivots} />
            <Metric label="Validaciones" value={health.validations} />
            <Metric label="Comentarios" value={health.comments} />
            <Metric label="Rangos protegidos" value={health.protectedRanges} />
            <Metric label="Nombres definidos" value={health.namedRanges} />
            <Metric
              label="Warnings import/XLSX"
              value={health.importWarnings + health.unsupportedXlsxFeatures}
            />
            <div className="mt-3 space-y-2">
              {health.findings.slice(0, 5).map((f: any) => (
                <div
                  key={f.code}
                  className="rounded-xl bg-white p-2 shadow-sm dark:bg-white/[0.04]"
                >
                  <b>{f.severity}</b> · {f.message}
                </div>
              ))}
              {!health.findings.length && (
                <div className="rounded-xl bg-white p-2 shadow-sm dark:bg-white/[0.04]">
                  Sin hallazgos relevantes para compartir/exportar.
                </div>
              )}
            </div>
          </Panel>
        ...
        {tab === "cell" && (
          <Panel title="Selección / celda">
            <Metric label="Rango" value={selection.range} />
            <Metric label="Count" value={selection.count} />
            <Metric label="Números" value={selection.nums} />
            <Metric label="Sum" value={selection.sum.toLocaleString()} />
            <Metric
              label="Average"
              value={selection.average.toLocaleString()}
            />
            <Metric label="Min" value={selection.min ?? "—"} />
            <Metric label="Max" value={selection.max ?? "—"} />
            <Metric label="Fórmulas" value={selection.formulas} />
            <Metric label="Comentarios" value={selection.comments} />
            <Metric
              label="Estado"
              value={selection.protected ? "Protected" : "Ready"}
            />
          </Panel>
        )}
        {tab === "data" && (
          <Panel title="Data Tools">
            ...
            >
              Validación de datos — reglas por rango
            </button>
            <button
              disabled={readOnly}
              className={tool}
            ...
            >
              Formato condicional — resaltar riesgos
            </button>
            <button disabled={readOnly} className={tool} onClick={onOpenNames}>
              Nombres definidos — manager
            </button>
            <button
              disabled={readOnly}
              className={tool}
            ...
            >
              Goal Seek — objetivo financiero/OEE
            </button>
            <button disabled={readOnly} className={tool} onClick={onOpenSolver}>
              Solver — optimización restringida
            </button>
          </Panel>
        )}
        {tab === "charts" && (
          <Panel title="Charts">
            <button disabled={readOnly} className={tool} onClick={onOpenChart}>
              Insertar sparkline desde rango
            </button>
            {charts.map((c: any) => (
              <div
                key={c.id}
            ...
            {!charts.length && (
              <Empty text="No hay charts persistidos; usa Insertar para construir visualizaciones." />
            )}
          </Panel>
        )}
        {tab === "pivot" && (
          <Panel title="Pivot Workbench">
            <button disabled={readOnly} className={tool} onClick={onOpenPivot}>
              Crear tabla dinámica desde selección
            </button>
            {pivots.map((p: any) => (
              <div
                key={p.id}
              ...
              >
                <b>{p.sheetName}</b>
                <br />
                Rows {(p.config?.rows ?? []).join(", ") || "—"} · Values{" "}
                {(p.config?.values ?? []).length}
              </div>
            ))}
            {!pivots.length && (
              <Empty text="Sin definiciones de pivot guardadas." />
            )}
          </Panel>
        )}
        {tab === "comments" && (
          <Panel title="Comments">
            <button disabled={readOnly} className={tool} onClick={onAddComment}>
              Agregar comentario a {selection.range}
            </button>
            <div className="flex gap-1">
              {(["open", "resolved", "assigned"] as const).map((f) => (
                <button
                  key={f}
                  onClick={() => setCommentFilter(f)}
                  className={`rounded-lg px-2 py-1 text-[11px] font-semibold ${commentFilter === f ? "bg-emerald-600 text-white" : "bg-white dark:bg-white/[0.04]"}`}
                >
                  {f === "open"
                    ? `Abiertos ${openComments.length}`
                    : f === "resolved"
                      ? "Resueltos"
                      : "Asignados"}
                </button>
            ...
            {visibleComments.map((c: any) => (
              <div
                key={c.id}
                className={`rounded-xl p-2 shadow-sm ${c.resolved ? "bg-emerald-500/10" : "bg-white dark:bg-white/[0.04]"}`}
              >
                <b>{sheets[c.sheetIndex]?.name || "Hoja"}</b> ·{" "}
                {c.anchorType || "range"} · {c.range}
                <br />
                {c.text || c.messages?.[0]?.text || "Comentario"}
                {c.assignedTo ? (
                  <div className="mt-1 text-[11px] font-semibold text-blue-500">
                    Asignado a {c.assignedTo}
                  </div>
                ) : null}
              </div>
            ))}
            {!visibleComments.length && (
              <Empty text="No hay comentarios para este filtro." />
            )}
          </Panel>
        )}
        {tab === "protection" && (
          <Panel title="Protection">
            ...
            {protectedRanges.map((p: any, i: number) => (
              <div
                key={i}
                className="rounded-xl bg-white p-2 shadow-sm dark:bg-white/[0.04]"
              >
            ...
            {!protectedRanges.length && (
              <Empty text="La hoja activa no tiene protección AXOS visible." />
            )}
          </Panel>
        )}
        {tab === "xlsx" && (
          <Panel title="XLSX Compatibility Review">
            <Metric label="Score" value={`${compatibility.score}/100`} />
            <Metric label="Revisión" value={compatibility.reviewCount} />
            <Metric
              label="No soportado"
              value={compatibility.unsupportedCount}
            />
            <div className="mt-3 space-y-2">
              {compatibility.features.map((f: any) => (
                <div
                  key={f.key}
                  className="rounded-xl bg-white p-2 shadow-sm dark:bg-white/[0.04]"
                >
                  <b>{f.label}</b> · {f.count} · {f.severity}
                  <br />
        ...
        {tab === "axos" && (
          <Panel title="AXOS Data">
            <button className={tool} onClick={onRefreshConnectors}>
              Refrescar conectores insertados
            </button>
            <div className="grid grid-cols-1 gap-2">
              {AXOS_SHEET_CONNECTORS.map((c) => (
                <button
                  key={c.type}
                  disabled={readOnly}
                  onClick={() => onInsertConnector(c.type)}
                  className={tool}
                >
                  <b>{c.label}</b>
                  <br />
            ...
            <div className="mt-3 text-[11px] text-gray-500">
              Contratos reales ERP/MES: se insertan solo conectores soportados
              por metadata AXOS; endpoints externos quedan pendientes de
              contrato, sin simular refresh.
            </div>
            {connectors.map((c: any) => (
              <div
                key={c.id}
                className="mt-2 rounded-xl bg-white p-2 shadow-sm dark:bg-white/[0.04]"
              >
                <b>{c.label}</b> · {c.range}
                <br />
                Último refresh {c.lastRefreshedAt || "—"}
              </div>
            ))}
          </Panel>
        )}
      </div>
      <div className="border-t border-black/10 dark:border-white/10 p-3 text-[11px] text-gray-500">
        Hoja activa: {activeSheet.name || `Hoja ${activeSheetIndex + 1}`} ·{" "}
        {names.length} nombres · {connectors.length} conectores ·{" "}
        {openComments.length} comentarios abiertos
      </div>
    </aside>
  );
}
...
function Metric({ label, value }: any) {
  return (
    <div className="flex items-center justify-between border-b border-black/5 py-1 dark:border-white/10">
      <span>{label}</span>
...
function Empty({ text }: { text: string }) {
  return (
    <div className="rounded-xl border border-dashed border-black/10 p-3 text-gray-500 dark:border-white/10">
      {text}
    </div>
  );
}
