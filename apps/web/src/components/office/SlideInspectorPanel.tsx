      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-3 space-y-3">
        <Section title="Deck health" icon={<Activity className="w-3.5 h-3.5" />}>
          ...
          <div className="mt-2 text-[11px] text-gray-500 dark:text-gray-400 space-y-1">
            ...
            <p><Timer className="mr-1 inline h-3 w-3" />Transitions: <b>{health.transitionSlides}</b> slide(s), <b>{health.transitionVariants}</b> style(s) · Auto advance: <b>{health.autoAdvanceSlides}</b></p>
          </div>
...
function Metric({ label, value, tone }: { label: string; value: number; tone?: 'green' | 'amber' | 'rose' }) { return <div className={`rounded-lg p-2 border ${tone === 'amber' ? 'border-amber-300/50 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300' : tone === 'rose' ? 'border-rose-300/50 bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-300' : tone === 'green' ? 'border-emerald-300/50 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : 'border-black/10 dark:border-white/10'}`}><div className="text-lg font-bold leading-none">{value}</div><div className="text-[10px] opacity-70">{label}</div></div>; }
