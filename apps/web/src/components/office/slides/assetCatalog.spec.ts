...
ok(SLIDE_ASSET_USE_CASES.length >= 8, 'asset use-case facets cover industrial deck workflows');
...
for (const asset of INDUSTRIAL_ASSETS) {
  ...
  ok(metadata.useCases.length > 0, `${asset.id}: has workflow metadata`);
  ok(metadata.altText.includes(asset.label), `${asset.id}: alt text references the asset label`);
  ok(metadata.pptxFidelity === 'svg-vector', `${asset.id}: declares SVG vector PPTX fidelity`);
}
...
for (const useCase of SLIDE_ASSET_USE_CASES) {
  ok(stats.useCases[useCase.id] > 0, `${useCase.id}: use-case has at least one asset`);
}
...
ok(filterSlideAssets(INDUSTRIAL_ASSETS, { query: 'visual aid' }).some((asset) => getSlideAssetUseCases(asset).includes('visualAid')), 'search includes use-case labels');
...
ok(filterSlideAssets(INDUSTRIAL_ASSETS, { useCase: 'supplierReview' }).every((asset) => getSlideAssetUseCases(asset).includes('supplierReview')), 'use-case filter scopes supplier review assets');
...
eq(ASSET_USE_CASE_LABEL[getSlideAssetUseCases(getSlideAssetById('supplier-scorecard')!)[0]], 'Supplier review', 'use-case labels are exposed for cards');
