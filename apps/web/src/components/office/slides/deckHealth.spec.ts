...
/** Slides deck health smoke spec. Run from apps/web with: npx tsx src/components/office/slides/deckHealth.spec.ts */
  ...
  },
  {
    objects: [
      ...
      { type: 'rect', left: 40, top: 120, width: 200, height: 140, visible: false, locked: true },
      {
        ...
        smartObject: {
          title: 'Supplier risk',
          source: 'AXOS.suppliers.scorecard',
          binding: { lastUpdatedAt: '2026-06-29T18:00:00.000Z' },
        },
        left: 300,
        top: 120,
        width: 260,
...
eq(health.sectionCount, 2, 'counts sections');
...
eq(health.exportReadiness, 'blocked', 'empty and off-canvas content block export');
...
eq(clean.exportReadiness, 'ready', 'clean deck is export ready');
eq(clean.readinessIssues, [], 'clean deck has no readiness issues');

if (failures.length) {
  console.error(`deck health spec failed: ${failures.length}/${passed + failures.length}`);
