...
import { deriveFormulaBarState, deriveNameBoxLabel, deriveSheetSelectionStats, deriveSheetSummary, deriveWorkbookHealth, formatSheetRange } from './workbookHealth';
...
eq(deriveNameBoxLabel(workbook.names, 0, 'A1:B1'), 'Revenue', 'name box muestra nombre definido');
eq(deriveFormulaBarState({ names: workbook.names, sheetIndex: 0, range: 'A1:B1', selection: stats }).mode, 'Protected', 'formula bar prioriza protección');
