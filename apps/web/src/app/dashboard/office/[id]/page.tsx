...
import {
  ...
  type SaveStatus,
  type OfficeType,
} from "@/components/office/OfficeShell";
...
import {
  DocLifecycleActions,
  type OfficeLifecycleState,
} from "@/components/office/DocLifecycleActions";
...
const DocEditor = dynamic(
  () => import("@/components/office/DocEditor").then((m) => m.DocEditor),
  { ssr: false, loading: Spinner },
);
const SheetEditor = dynamic(
  () => import("@/components/office/SheetEditor").then((m) => m.SheetEditor),
  { ssr: false, loading: Spinner },
);
const SlidesEditor = dynamic(
  () => import("@/components/office/SlidesEditor").then((m) => m.SlidesEditor),
  { ssr: false, loading: Spinner },
);
...
export default function OfficeEditorPage() {
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoading(true);
    apiFetch(`${API_BASE}/office-documents/${id}`)
      .then(async (r) => {
        if (!active) return;
        if (!r.ok) {
          setError(
      ...
      .catch(() => {
        if (active) {
          setError("Error de red al abrir el documento.");
          setLoading(false);
        }
      });
    return () => {
  ...
  if (loading || authLoading || !doc) {
    return (
      <OfficeShellMessage>
        ...
        <p className="text-sm text-gray-500">Abriendo documento…</p>
      </OfficeShellMessage>
    );
  }
  ...
  const actions = (
    <>
      ...
      {isOwner && canWrite && (
        <ShareButton docId={id} initialShares={doc.sharedWith ?? []} />
      )}
      <VersionHistory
        docId={id}
        canEdit={!readOnly}
  ...
  const statusBarRight =
    doc.type === "doc" && docStats ? (
      <span>
        {docStats.words} palabras · {docStats.chars} caracteres
      </span>
    ) : null;
  ...
  const ribbonArchivo =
    doc.type === "doc" || doc.type === "sheet" || doc.type === "slides";
  ...
  return (
    ...
    >
      {doc.type === "doc" ? (
        ...
        <SlidesEditor key={editorKey} {...editorProps} fileActions={actions} />
      ) : (
