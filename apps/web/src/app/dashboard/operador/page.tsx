...
interface GenealogyAsBuiltComponent {
  part: string;
  totalQty: number;
  lots: string[];
  reels: string[];
...
interface GenealogyAsBuiltView {
  serial: string;
  model: string | null;
  woId: string | null;
  woFolio: string | null;
  componentCount: number;
  parts: GenealogyAsBuiltComponent[];
...
interface GenealogyWhereUsedView {
...
interface CertificationCheckView {
  certified: boolean;
...
interface Board {
  ...
  recentEvents: ExecutionTraceEvent[];
...
function useOfflineActionQueue() {
  ...
  const markAttempt = useCallback((id: string, lastError?: string) => {
    setQueue((prev) =>
      prev.map((item) =>
        item.id === id
  ...
  return { queue, enqueue, clear, remove, markAttempt };
}
...
export default function OperadorPage() {
  ...
  const [incidentDraft, setIncidentDraft] =
    useState<ContainmentIncidentDraft | null>(null);
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 pt-6">
        ...
        {!forbidden && executionId && board && (
          <BoardView
            ...
            onCreateContainmentIncident={(draft) => {
            ...
            removeOfflineAction={offlineQueue.remove}
            socketStatus={socketStatus}
          />
        )}
...
function BoardView({
  ...
  markOfflineAttempt: (id: string, lastError?: string) => void;
  ...
  const certificationPath = currentStep
    ? `/people/certification-check?employee=${encodeURIComponent(
        operator,
      )}&station=${encodeURIComponent(currentStep.name)}`
    : null;
  const { data: certification } = useApi<CertificationCheckView>(
    certificationPath,
  );
  const readiness = deriveOperatorReadiness({
    hasWorkOrder: !!execution,
    hasCurrentStep: !!currentStep,
    stepStatus: currentStep?.status ?? null,
    starved: currentStep?.starved,
    maxConfirmable: currentStep?.maxConfirmable ?? 0,
    materialShortCount:
      currentStepDetail?.materials.filter((material) => material.short).length ?? 0,
    openIncidentCount: currentStepDetail?.openIncidents.length ?? 0,
    blockingIncidentCount:
      currentStepDetail?.openIncidents.filter((incident) => incident.blocksFlow)
        .length ?? 0,
    openAndonCount: board.andons.filter((andon) => andon.status !== "resolved")
      .length,
    openDowntimeCount: board.openDowntime.length,
    hasInstructions: !!currentStepDetail?.instructions,
    hasVisualAid: !!currentStepDetail?.visualAid,
    assignedOperatorNames: board.assignments.map(
  ...
  const { data: ledgerEvents } = useApi<LedgerEventView[]>(
    `/ledger/work-order/${encodeURIComponent(execution.workOrder)}?limit=20`,
  );
  const activeSerial =
    board.recentEvents.find((event) => event.serial)?.serial ?? null;
  const [genealogySerial, setGenealogySerial] = useState<string | null>(null);
  const serialToInspect = genealogySerial || activeSerial;
  const { data: asBuiltGenealogy } = useApi<GenealogyAsBuiltView>(
    serialToInspect
      ? `/genealogy/as-built/by-serial/${encodeURIComponent(serialToInspect)}`
      : null,
  );
  const activeLot = board.recentEvents.find((event) => event.lot)?.lot ?? null;
  const [whereUsedLot, setWhereUsedLot] = useState<string | null>(null);
  const lotToInspect = whereUsedLot || activeLot;
  const { data: whereUsedGenealogy } = useApi<GenealogyWhereUsedView>(
    lotToInspect
      ? `/genealogy/where-used/by-lot?lot=${encodeURIComponent(lotToInspect)}`
      : null,
  );

  const replayOfflineAction = useCallback(
    async (action: OfflineAction) => {
      if (["confirm", "incident", "andon"].includes(action.type)) {
        const validation = validateOperatorTransaction({
          type: action.type as "confirm" | "incident" | "andon",
          payload: action.payload,
        });
        if (!validation.ok) {
          markOfflineAttempt(action.id, validation.errors.join(" · "));
          return;
        }
      }
      if (!action.endpoint) {
        markOfflineAttempt(action.id, "Acción legacy sin endpoint");
        return;
      }
      markOfflineAttempt(action.id);
      try {
        const res = await apiFetch(`${API_BASE}${action.endpoint}`, {
        ...
        markOfflineAttempt(action.id, "Sin conexión");
      }
    },
    [markOfflineAttempt, refresh, removeOfflineAction],
  );

  const replayOfflineQueue = useCallback(async () => {
    const replayable = offlineQueue.filter((action) =>
      ["confirm", "incident", "andon"].includes(action.type),
    );
    if (!replayable.length) return;

    const validActions = replayable.filter((action) => {
      const validation = validateOperatorTransaction({
        type: action.type as "confirm" | "incident" | "andon",
        payload: action.payload,
      });
    ...
    validActions.forEach((action) => markOfflineAttempt(action.id));
    try {
      const res = await apiFetch(`${API_BASE}/mes/offline/replay`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        validActions.forEach((action) =>
          markOfflineAttempt(action.id, body.message ?? `HTTP ${res.status}`),
        );
        return;
      }
      for (const result of body.results ?? []) {
        if (result.ok) removeOfflineAction(result.id);
        else markOfflineAttempt(result.id, result.message ?? "Replay rechazado");
      }
      refresh();
    } catch {
      validActions.forEach((action) =>
  ...
  }, [markOfflineAttempt, offlineQueue, refresh, removeOfflineAction]);
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <MaterialConsumptionPanel
          materials={currentStepDetail?.materials ?? []}
          kitId={execution.kitId ?? null}
          operator={operator}
      ...
      <ExecutionCommandCenter
        ...
        replayOfflineAction={replayOfflineAction}
        replayOfflineQueue={replayOfflineQueue}
        apiBase={API_BASE}
        ledgerEvents={ledgerEvents ?? []}
        asBuiltGenealogy={asBuiltGenealogy ?? null}
        genealogySerial={serialToInspect}
        onSelectGenealogySerial={setGenealogySerial}
        whereUsedGenealogy={whereUsedGenealogy ?? null}
        whereUsedLot={lotToInspect}
        onSelectWhereUsedLot={setWhereUsedLot}
        onCreateContainmentIncident={onCreateContainmentIncident}
      />
  ...
  async function submit() {
    ...
    try {
      ...
      onQueueAction({
        ...
        endpoint: `/mes/executions/${board.execution.id}/steps/${step.stepId}/confirm`,
        method: "POST",
        payload,
      });
...
function IncidentForm({
  ...
  board: Board;
  operator: string;
  draft?: ContainmentIncidentDraft | null;
  onQueueAction: (
    action: Omit<OfflineAction, "id" | "createdAt" | "attempts">,
  ) => void;
  onDone: () => void;
  ...
  const step = board.currentStep!;
  const [type, setType] = useState(draft?.type ?? INCIDENT_TYPES[0]);
  const [severity, setSeverity] = useState(draft?.severity ?? "medium");
  const [qty, setQty] = useState(1);
  const [blocks, setBlocks] = useState(draft?.blocksFlow ?? false);
  const [desc, setDesc] = useState(draft?.description ?? "");
  const [evidenceNote, setEvidenceNote] = useState("");
  const [signature, setSignature] = useState(operator);
  const [quickNcr, setQuickNcr] = useState(draft?.quickNcr ?? false);
  ...
  async function submit() {
    ...
    if (!validation.ok) {
      toast.error(validation.errors.join(" "), "Calidad");
      return;
    }
    setBusy(true);
    try {
      ...
      onQueueAction({
        ...
        endpoint: `/mes/executions/${board.execution.id}/steps/${step.stepId}/incidents`,
        method: "POST",
        payload,
      });
...
function AndonForm({
  ...
  async function call(type: string) {
    const payload = {
      ...
      raisedBy: operator,
      note: `Terminal operador · ${board.execution.workOrder}`,
      clientRequestId: reqId(),
    };
    const validation = validateOperatorTransaction({
      type: "andon",
    ...
    if (!validation.ok) {
      toast.error(validation.errors.join(" "), "Andon");
      return;
    }
    setBusy(type);
    try {
      ...
      onQueueAction({
        ...
        endpoint: `/mes/executions/${board.execution.id}/andon`,
        method: "POST",
        payload,
      });
