...
export class ResuppliesService {
  ...
  private async applyScopeToQb(
    qb: import('typeorm').SelectQueryBuilder<Resupply>,
    scope?: ScopeQuery,
  ): Promise<void> {
    if (!scope) return;
  ...
  private async applyScope(
    ...
    if (scope.buildingId) {
      ...
      lineAllowedLegacyNumbers = lines
        .map((line) => line.legacyLineNumber)
    ...
    return rows.filter((row) => {
      ...
      if (scope.model && !model.includes(scope.model.toUpperCase()))
        return false;
      if (scope.workOrder && !workOrder.includes(scope.workOrder.toUpperCase()))
        return false;
      if (lineAllowedLegacyNumbers && !lineAllowedLegacyNumbers.includes(line))
        return false;
      if (requiredProgramPrefix && !model.startsWith(requiredProgramPrefix))
        return false;
      return true;
    });
  }
  ...
  async updateStatus(
    ...
    if (dto.sourceWarehouseId)
      updateData.sourceWarehouseId = dto.sourceWarehouseId;
    if (dto.sourceLocation) updateData.sourceLocation = dto.sourceLocation;
    if (dto.destinationWarehouseId)
      updateData.destinationWarehouseId = dto.destinationWarehouseId;
    if (dto.destinationLocation)
      updateData.destinationLocation = dto.destinationLocation;
    ...
    if (dto.status === 'pick_completed')
      updateData.pickCompletedAt = new Date();
    ...
    try {
      if (dto.status === 'pick_started') {
        ...
        await this.inventory.recordTransaction({
          ...
          quantity: qty,
          fromWarehouseId:
            dto.sourceWarehouseId || resupply.sourceWarehouseId || 'WH-CENTRAL',
      ...
      if (dto.status === 'delivered') {
        ...
        await this.inventory.recordTransaction({
          type: (
            ...
            resupply.destinationWarehouseId ||
            ''
          ).includes('LINE')
            ? 'ISSUE'
            : 'TRANSFER',
          ...
          toWarehouseId:
            ...
            'WH-LINE',
          toLocation:
            dto.destinationLocation || resupply.destinationLocation || 'LINE',
          actorName: dto.actorName,
          referenceType: 'RESUPPLY',
          referenceId: id.toString(),
    ...
    await this.eventLedger.recordEvent({
      domain: EventDomain.MATERIALS,
      action:
        actionMap[dto.status] || `RESUPPLY_STATUS_${dto.status.toUpperCase()}`,
      actorName: dto.actorName,
      referenceType: 'RESUPPLY',
      referenceId: id.toString(),
      customer: (updated.kit.plan as any)?.customer,
  ...
  private async updateMaterialQuantities(resupply: Resupply) {
    ...
    const totalResupplied = allDelivered.reduce((sum, r) => {
      return r.status === 'delivered' || r.status === 'confirmed'
        ? sum + (r.quantityDelivered ?? 0)
        : sum;
    }, 0);
