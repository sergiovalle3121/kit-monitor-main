...
import {
  WarehouseTaskStatus,
  WarehouseTaskType,
} from '../inventory/entities/warehouse-task.entity';
import { ShipmentStatus } from '../shipping/entities/shipment.entity';

@Injectable()
export class DecisionIntelligenceService {
  constructor(
    @InjectRepository(ForecastRun)
    private readonly runRepo: Repository<ForecastRun>,
    @InjectRepository(ForecastSeriesResult)
    private readonly seriesRepo: Repository<ForecastSeriesResult>,
    @InjectRepository(PlanScenario)
    private readonly scenarioRepo: Repository<PlanScenario>,
    @InjectRepository(PlanPublication)
    private readonly publicationRepo: Repository<PlanPublication>,
    ...
    @InjectRepository(ScenarioSimulationResult)
    private readonly simulationRepo: Repository<ScenarioSimulationResult>,
    @InjectRepository(PlanActualOutcome)
    private readonly outcomeRepo: Repository<PlanActualOutcome>,
    @InjectRepository(ScoreCalibrationPoint)
    private readonly calibrationRepo: Repository<ScoreCalibrationPoint>,
    ...
    private readonly warehouseTaskRepo: Repository<WarehouseTask>,
  ...
  async createForecastRun(dto: CreateForecastRunDto) {
    if (!dto.series?.length)
      throw new BadRequestException(
        'Se requiere al menos una serie para crear run',
      );
      ...
      this.runRepo.create({
        ...
        series: dto.series.map((row) =>
          this.seriesRepo.create({
            ...
            confidenceScore:
              row.confidenceScore ?? this.scoreSeries(row.mape, row.bias),
          }),
        ),
      }),
    );
  ...
  async createPlanScenario(dto: CreatePlanScenarioDto) {
    const run = dto.runId
      ...
      : null;
    if (dto.runId && !run)
      throw new NotFoundException('Forecast run no encontrado');

    const logisticRisk = await this.buildLogisticPriority(
      run?.id ?? null,
      dto.assumptions.horizonDays,
      dto.assumptions.leadTimeDays ?? 2,
    );
    const capacityPressure =
      dto.assumptions.dailyCapacityUnits > 0
        ? dto.assumptions.plannedDemandUnits /
          (dto.assumptions.dailyCapacityUnits *
            Math.max(1, dto.assumptions.horizonDays))
        : 1;
    const forecastConfidence = run?.confidenceScore ?? 55;
    const riskPenalty = Math.min(40, logisticRisk.criticalCount * 6);
    const capacityPenalty = Math.min(
      30,
      Math.max(0, (capacityPressure - 0.85) * 100),
    );
    const scrapPenalty = Math.min(
      10,
      Math.max(0, (dto.assumptions.scrapRate ?? 0) * 100),
    );

    const viabilityScore = Math.max(
      5,
      Math.round(
        forecastConfidence - riskPenalty - capacityPenalty - scrapPenalty,
      ),
    );
    const estimatedProbability = Math.max(
      0.05,
      Math.min(0.98, viabilityScore / 100),
    );
  ...
  async publishPlan(dto: CreatePlanPublicationDto) {
    const run = dto.runId
      ? await this.runRepo.findOne({ where: { id: dto.runId } })
      : null;
    const scenario = dto.scenarioId
      ? await this.scenarioRepo.findOne({ where: { id: dto.scenarioId } })
      : null;
    if (dto.runId && !run)
      throw new NotFoundException('Forecast run no encontrado');
    if (dto.scenarioId && !scenario)
      throw new NotFoundException('Plan scenario no encontrado');

    const publication = await this.publicationRepo.save(
      this.publicationRepo.create({
        title: dto.title,
        run: run ?? undefined,
        scenario: scenario ?? undefined,
        planConfidenceScore:
          scenario?.viabilityScore ?? run?.confidenceScore ?? 0,
  ...
  async runScenarioSimulation(scenarioId: number, dto: RunSimulationDto) {
    ...
    const historicalErrors = run
      ? await this.errorHistoryRepo.find({ where: { run: { id: run.id } } })
      : [];
    const residualPool = historicalErrors.map((item) => item.residual);
    const dataSufficiencyScore = Math.min(
      100,
      Math.round((historicalErrors.length / 60) * 100),
    );
    const simulationMode =
      residualPool.length >= 20 ? 'monte_carlo' : 'fallback_band';
    ...
    for (let i = 0; i < numRuns; i++) {
      ...
      const scrap = Math.max(
        ...
        scrapBase +
          this.randomBetween(
            -(dto.scrapStdPct ?? 0.015),
            dto.scrapStdPct ?? 0.015,
          ),
      );
      const capacityNoise = this.randomBetween(
        -(dto.capacityStdPct ?? 0.12),
        dto.capacityStdPct ?? 0.12,
      );
      const simulatedCapacity =
        capacityPerDay * horizonDays * (1 + capacityNoise);
    ...
    const result = await this.simulationRepo.save(
      this.simulationRepo.create({
  ...
  async registerPublicationOutcome(
    publicationId: number,
    dto: RegisterOutcomeDto,
  ) {
    ...
    const outcome = await this.outcomeRepo.save(
  ...
  async getControlTower(publicationId: number) {
    ...
    const simulation = publication.scenario?.id
      ? await this.simulationRepo.findOne({
          where: { scenario: { id: publication.scenario.id } },
          order: { createdAt: 'DESC' },
        })
      : null;
  ...
  async getCalibrationSummary() {
    const points = await this.calibrationRepo.find({
      order: { bucket: 'ASC' },
    });
    ...
    const successRate = outcomes.length
      ? outcomes.filter((row) => row.fulfillmentResult === 'success').length /
        outcomes.length
      : 0;
    return {
  ...
  private scoreRunConfidence(series: CreateForecastRunDto['series']): number {
    const avg =
      series.reduce(
        (acc, row) => acc + this.scoreSeries(row.mape, row.bias),
        0,
  ...
  private async buildLogisticPriority(
    runId: number | null,
    horizonDays: number,
    leadTimeDays: number,
  ) {
    ...
    const items = materials
      .map((item) => {
        ...
        const consumptionPerMinute =
          demand > 0 ? demand / horizonMinutes : item.usagePerAssembly / 60;
        ...
        const leadGap =
          minutesToStockout === null
            ? 0
            : Math.max(0, leadTimeMinutes - minutesToStockout);
    ...
    return {
      criticalCount: items.filter(
        (item) => item.severity === 'urgent' || item.severity === 'prepare',
      ).length,
      leadTimeDays,
      horizonDays,
      items,
  ...
  private async seedErrorHistoryFromRun(runId: number): Promise<void> {
    ...
    const rows = run.series.map((row) => {
      const actual = Math.max(
        0,
        row.forecastNext +
          this.randomBetween(-Math.max(2, row.mad), Math.max(2, row.mad)),
      );
      const residual = actual - row.forecastNext;
      const absoluteError = Math.abs(residual);
      const percentageError =
        row.forecastNext > 0 ? (absoluteError / row.forecastNext) * 100 : 0;
  ...
  private async updateCalibrationSummary(): Promise<void> {
    ...
    const points = [...buckets.entries()].map(([bucket, slot]) => {
      ...
      const observed =
        slot.success.reduce((a, b) => a + b, 0) / slot.success.length;
      const calibrated = this.calibrateScore(
        avgRaw,
        Math.min(100, slot.raw.length * 10),
        observed,
      );
  ...
  private buildSignals(
    ...
    outcome: PlanActualOutcome | null,
    simulation: ScenarioSimulationResult | null,
  ) {
    ...
    if (outcome.shortageEvents > 0)
      signals.push('Riesgo creciente por faltantes');
    if (
      simulation &&
      simulation.probabilityOfPlanSuccess < 0.6 &&
      outcome.fulfillmentResult === 'success'
    )
      signals.push('Riesgo mitigado en ejecución');
    return signals;
  }
  ...
  private calibrateScore(
    ...
    return this.round(
      rawScore * (0.6 * suffWeight) +
        observedSuccess * 100 * (1 - 0.6 * suffWeight),
    );
  }
  ...
  async getSiteOverview() {
    ...
    const tasks = await this.warehouseTaskRepo.find({
      where: { status: WarehouseTaskStatus.PENDING },
    });
    ...
    const pendingOqcQty = inventory
      .filter((p) => p.holdStatus === 'pending_oqc')
      .reduce((acc, p) => acc + p.onHand, 0);
    const blockedFgQty = inventory
      .filter((p) => p.holdStatus === 'hold' && p.warehouseId === 'WH-FG')
      .reduce((acc, p) => acc + p.onHand, 0);

    // 3. Supply Chain Signals
    const shortages = inventory.filter((p) => p.onHand <= 0).length;
    const quarantinedQty = inventory
      .filter((p) => p.holdStatus === 'quarantine')
      .reduce((acc, p) => acc + p.onHand, 0);
    ...
    const criticalSuppliers = suppliers.filter(
      (s) => s.status === 'Critical',
    ).length; // Assuming status exists

    return {
      site: {
        id: 'DEFAULT-CAMPUS',
        name: 'Default Industrial Campus',
        healthScore: this.calculateDeepHealth(
          ncrs,
          tasks,
          wip,
          fpy,
      ...
      quality: {
        ...
        iqcPassRate:
          iqc.length > 0
            ? this.round(
                (iqc.filter((i) => i.result === IqcResult.PASS).length /
                  iqc.length) *
                  100,
              )
            : 100,
      ...
      materials: {
        ...
        quarantinedQty,
        holdQty: inventory
          .filter((p) => p.holdStatus === 'hold')
          .reduce((acc, p) => acc + p.onHand, 0),
        replenishmentBacklog: tasks.filter(
          (t) => t.type === WarehouseTaskType.TRANSFER,
        ).length,
      },
      production: {
        activeWorkOrders: wip.filter((w) => w.status === 'in_production')
          .length,
      ...
      shipping: {
        ...
        blockers: shipments.filter(
          (s) =>
            s.status === ShipmentStatus.PLANNING &&
            s.scheduledAt &&
      ...
      suppliers: {
        ...
        openScars,
        avgScarAging: this.calculateAvgAging(
          scars.filter((s) => s.status !== ScarStatus.CLOSED),
        ),
      },
    };
  }

  private calculateDeepHealth(
    ncrs: any[],
    tasks: any[],
    wip: any[],
    fpy: number,
  ...
  private calculateAvgAging(items: any[]): number {
    ...
    const sum = items.reduce(
      (acc, item) => acc + (now - new Date(item.createdAt).getTime()),
      0,
    );
