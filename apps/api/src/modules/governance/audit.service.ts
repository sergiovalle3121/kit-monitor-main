...
export class AuditService {
  ...
  async recordAction(params: {
    ...
    if (params.outcome === 'DENIED') {
      await this.recordException({
        ...
        description:
          params.reason ||
          `User ${params.actor} attempted unauthorized ${params.action} on ${params.resourceType} ${params.resourceId}`,
        actor: params.actor,
  ...
  async findAllExceptions(
    ...
    if (filters.domain)
      qb.andWhere('ex.domain = :domain', { domain: filters.domain });
    if (filters.severity)
      qb.andWhere('ex.severity = :severity', { severity: filters.severity });
    if (filters.status)
      qb.andWhere('ex.status = :status', { status: filters.status });
  ...
  async updateExceptionStatus(
    ...
    if (status === ExceptionStatus.ACKNOWLEDGED)
      return this.acknowledgeException(id, actor);
    if (status === ExceptionStatus.RESOLVED)
      return this.resolveException(id, actor, {
        reason: 'Status updated via bulk action',
      });
  ...
  async checkEscalations() {
    const now = new Date();
    const overdue = await this.exceptionRepo
    ...
    for (const ex of overdue) {
      ...
      if (
        ...
        const thresholdHours =
          policy?.escalationThresholdHours ??
          (ex.severity === ExceptionSeverity.CRITICAL ? 1 : 4);
        const escalationTime = new Date(
          ex.dueAt.getTime() + thresholdHours * 60 * 60 * 1000,
        );
