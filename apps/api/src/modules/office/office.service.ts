...
import {
  ...
  OfficeCommentReply,
} from './entities/office-document-comment.entity';
import {
...
interface GenericCommentPatchDto {
  resolved?: boolean;
  assignedTo?: string | null;
  anchorLabel?: string | null;
  rangeRef?: string | null;
...
export class OfficeService {
  constructor(
    ...
    @InjectRepository(OfficeDocumentComment)
    private readonly commentRepo: Repository<OfficeDocumentComment>,
    private readonly audit: AuditService,
    @InjectRepository(OfficeComment)
    private readonly slideCommentRepo: Repository<OfficeComment>,
  ) {}
  ...
  async listComments(
    ...
    const mention = String(query.mention ?? '')
      ...
      .toLowerCase();
    if (mention)
      qb.andWhere('LOWER(CAST(c.mentions AS TEXT)) LIKE :mention', {
        mention: `%${mention}%`,
      });
      ...
      qb.andWhere(
        new Brackets((b) => {
          b.where('LOWER(c.text) LIKE :q', { q: `%${q}%` })
            .orWhere("LOWER(COALESCE(c.quotedText, '')) LIKE :q", {
              q: `%${q}%`,
            })
  ...
  async updateComment(
    ...
    if (dto.mentions !== undefined)
      c.mentions = this.normalizeMentions(dto.mentions);
    ...
    if (dto.quotedText !== undefined)
      c.quotedText = String(dto.quotedText ?? '').slice(0, 1000) || null;
  ...
  async replyToComment(
    ...
    const reply: OfficeCommentReply = {
  ...
  async listSlideComments(
    ...
    anchorTypes?: OfficeCommentAnchorType[],
  ) {
    const doc = await this.get(id, user);
    const qb = this.slideCommentRepo
      .createQueryBuilder('c')
      .where('c.documentId = :id', { id: doc.id })
      .orderBy('c.createdAt', 'ASC');
    if (anchorTypes?.length)
      qb.andWhere('c.anchorType IN (:...anchorTypes)', { anchorTypes });
    if (doc.tenantId)
  ...
  async addSlideComment(id: string, dto: CommentDto, user: AuthenticatedUser) {
    ...
    if (!text)
      throw new BadRequestException('El comentario no puede estar vacío.');
    if (dto.parentId) {
      const parent = await this.slideCommentRepo.findOne({
        where: { id: dto.parentId, documentId: doc.id },
      });
      if (!parent)
        throw new BadRequestException('Thread de comentario inválido.');
    }
    return this.slideCommentRepo.save(
      this.slideCommentRepo.create({
        documentId: doc.id,
  ...
  async updateSlideComment(
    id: string,
    commentId: string,
    dto: GenericCommentPatchDto,
    user: AuthenticatedUser,
  ) {
    const doc = await this.get(id, user);
    if (!this.canEdit(doc, user))
      throw new ForbiddenException(
        'No puedes actualizar comentarios en este documento.',
      );
    const c = await this.slideCommentRepo.findOne({
    ...
    if (dto.assignedTo !== undefined)
      c.assignedTo = this.normalizePrincipal(dto.assignedTo);
    if (dto.anchorLabel !== undefined)
      c.anchorLabel = String(dto.anchorLabel ?? '').trim() || null;
    if (dto.rangeRef !== undefined)
      c.rangeRef = String(dto.rangeRef ?? '').trim() || null;
  ...
  async resolveSlideComment(
    ...
    return this.updateSlideComment(id, commentId, { resolved }, user);
  }
