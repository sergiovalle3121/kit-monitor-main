...
| Warehouse rack generator | Yes | `apps/web/src/lib/cad/warehouse-generator.ts`, `Layout3DEditor.tsx` | usable | Generates rack rows and forklift aisles from local inputs; no saved generator presets yet. | Add supermarket/dock generators or persist generator presets after contract review. | `warehouse-generator.ts`, editor equipment rail | Medium: small editor insertion while PR 746 also edits `Layout3DEditor.tsx`. |
...
Open CAD PR #746 touches the main editor, minimap, and scale helpers. This run avoided viewport/minimap/DXF/command-registry/layer-model work and selected the next documented generator gap. The new `warehouse-generator.ts` builds editable rack assets, forklift aisle paths, labels, layer assignments, and generated tags from user inputs, and the existing equipment rail applies them through the same refs/history/snapshot path as templates.
