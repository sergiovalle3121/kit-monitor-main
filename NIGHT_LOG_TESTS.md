...
- **Estado de la suite:** **VERDE — 65 suites / 395 tests** (`npm test`). Antes de
  la sesión: 56 suites / 305 tests. +9 specs nuevos, +90 tests, 0 rojos, 0 skips.
...
|-------------|--------|---------|--------|
| Statements  | 25.02% | 29.51%  | +4.49  |
| Branches    | 22.30% | 26.90%  | +4.60  |
| Functions   | 18.20% | 21.51%  | +3.31  |
| Lines       | 25.29% | 29.86%  | +4.57  |
...
- **quality** (`quality.service.spec.ts` +18): checkIsHeld (niveles), motor de CAPA,
...
- Métodos con `QueryRunner` aún sin test: `quality.createHold/releaseHold/
  executeDisposition/recordFinalInspection` (simular runner como en inventory).
